...
parser.add_option('--telescope',"--tele","-T", 
                dest='instrument',
                default=None,
...
# flat_list  = opt.flat
target_list= args
flat_list  = get_files_or_list(opt.flat)
bias_list  = get_files_or_list(opt.bias)
dark_list  = get_files_or_list(opt.dark)
print(flat_list)
print(bias_list)
print(dark_list)
print(args)
...
Key_all=yaml.safe_load(open(telescope_yml_path,"r").read())
print_table(Key_all)
...
fits_info=pd.DataFrame(columns=["FitsName",
                                "MJD",
                                "ObjectType",
                                "Filter",])
        ...
        for f in Filter_all:
            ...
            for mm in flat_MJD:
                ...
                for ii in flat_list_all:
                    # print(img_mjd[os.path.basename(ii)])
                    print(img_filter.keys())
