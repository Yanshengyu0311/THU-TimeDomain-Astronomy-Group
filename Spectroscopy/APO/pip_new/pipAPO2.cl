...
real   aperture       {1,prompt="Apall apterture"}
string cal_dir        {"/home/ysy/iraf/all_std/",prompt="Directory of standard stars"}
string star_name      {prompt="Name of standard star"}
string extin_file     {"/home/ysy/iraf/APO/APO_pipeline/apoextinct.dat",prompt="Extinction file"}
...
tmp=substr(obj_filename,1,i-1)+"_c"+substr(obj_filename,i,i+4)+"s"
imdelet(tmp)
del temp.fits 
del temp1.fits 
...
i=stridx(".",tmp)
tmp1=substr(tmp,1,i-1)+substr(tmp,i,i+4)+"s.ms"
imdelet(tmp1)

apall(input=tmp,output=tmp1,format="multispec",referen="",interac=yes,find=yes,recente=yes,resize=yes,edit=yes,trace=yes,fittrac=yes,extract=yes,extras=yes,review=yes,llimit=ap1,ulimit=ap2,t_funct="spline3",t_order=5,backgro="fit",weights="variance",pfit="fit2d")
...
tmp=substr(std_filename,1,i-1)+substr(std_filename,i,i+4)+"s.ms"
#tmp="2017ein_standard.0001r.ms.fits"
imdelet(tmp)

apall(input=std_filename,output=tmp,format="multispec",referen="",interac=yes,find=yes,recente=yes,resize=yes,edit=yes,trace=yes,fittrac=yes,extras=yes,review=yes,llimit=ap1,ulimit=ap2,t_funct="spline3",t_order=5,backgro="fit",weights="variance",pfit="fit2d")
...
del std
standard(input=tmp3,output="std",caldir=cal_dir,star_nam=star_name,observatory="APO",extinction=extin_file,answer=yes)
