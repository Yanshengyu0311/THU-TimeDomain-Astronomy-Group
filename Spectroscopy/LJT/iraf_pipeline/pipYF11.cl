...
real     gain               {0.33,prompt="Gain,0.34"}
...
###########################################################################################################################################
hedit(image="./bias/ljg*",fields="DETSEC",value="[1:2148,1:4612]",verify=no)
hedit(image="./flat/ljg*",fields="DETSEC",value="[1:2148,1:4612]",verify=no)
hedit(image="ljg*"       ,fields="DETSEC",value="[1:2148,1:4612]",verify=no)
hedit(image="./bias/ljg*",fields="CCDSEC",value="[1:2148,1:4612]",verify=no)
hedit(image="./flat/ljg*",fields="CCDSEC",value="[1:2148,1:4612]",verify=no)
hedit(image="ljg*"       ,fields="CCDSEC",value="[1:2148,1:4612]",verify=no)
...
{
zerocombine(input="./bias/ljg*",output="Zero",ccdtype="",combine="average",reject="minmax",rdnoise=readnoise,gain=gain)
imcopy (input="Zero.fits",output="./bias/Zero")#将本底文件复制bias下，
}
#为了节省电脑储存空间，把Zero放在bias，原来bias文件删除
del(files="./bias/ljg*")                    #删除本底文件
#图像减bias
del(files="./flat/ljg*//tz")
ccdproc(images="./flat/ljg*",output="./flat/ljg*//tz",ccdtype="",fixpix=no,oversca=yes,biassec="[2100:2148,50:4612]",trimsec=trim_region,flatcor=no,darkcor=no,zero="Zero",zerocor=yes,trim=yes)
###########################################################################################################################################
#图像除平场
print("**********************************************")
...
#平场合并
flatcombine(input="./flat/ljg*tz.fits",output="./flat/Flat",ccdtype='',combine="median",reject="avsigclip",process=no,scale="mode",rdnoise=readnoise,gain=gain)
noao
twodspec
longslit
#平场归一化（色散方向）
...
#图像除平场
ccdproc(images="@l1",output="@l2",ccdtype='',fixpix=no,oversca=yes,trim=yes,zerocor=yes,darkcor=no,flatcor=yes,biassec="[2100:2148,50:4612]",trimsec=trim_region,zero="Zero",flat="perFlat")
#############################################################################################################################################
